          ...
          .exec((err, conversation) => {
            ...
            if (!conversation) {
              ...
              newChatSchema.save((err, conversationInserted) => {
                ...
                if (err) {
                  reject(new Error('Couldn\'t save your message please try again later'));
                  return callback(new Error('Couldn\'t save your message please try again later'));
                }
      ...
      chatSchema.findOne(query)
        ...
        .exec((err, message) => {
          if (err || !message) {
            ...
            let messageObj = message.messages.id(messageId);
            if (!messageObj) {
              ...
              messageObj.delivered = [];
              messageObj.seen = [];
              message.save((err) => {
                if (err) {
                  reject(new Error('Failed to set  message as finished  please try again later'));
                  return callback(new Error('Failed to set  message as finished  please try again later'));
                }
          ...
          if (err || !message) {
            ...
            if (!messageObj) {
              ...
              if (options.is_group) {
                ...
                let sentArray = _.pluck(messageObj.sent, '_id');
                sentArray = sentArray.map((sentId) => {
                  return sentId.toString();
                });
                var sent = _.contains(sentArray, userId);
                if (sent) {
                  messageObj.sent.remove(userId);
                }

                let deliveredArray = _.pluck(messageObj.delivered, '_id');
                deliveredArray = deliveredArray.map((deliveredId) => {
                  return deliveredId.toString();
                });
                var delivered = _.contains(deliveredArray, userId);
                if (delivered) {
                  messageObj.delivered.remove(userId);
                }

                let seenArray = _.pluck(messageObj.seen, '_id');
                seenArray = seenArray.map((seenId) => {
                  return seenId.toString();
                });
                var seen = _.contains(seenArray, userId);
                if (seen) {
                  messageObj.seen.remove(userId);
                }
              }
        ...
        .exec((err, message) => {
          if (err || !message) {
            ...
            if (!messageObj) {
              ...
              var seen = _.contains(seenArray, userId);
              if (!seen) {
                messageObj.seen.push(userId);
              }
              message.save((err) => {
                if (err) {
                  reject(new Error('Failed to set  message as seen  please try again later'));
                  return callback(new Error('Failed to set  message as seen  please try again later'));
                }
        ...
        .exec((err, message) => {
          if (err || !message) {
            ...
            if (!messageObj) {
              ...
              var delivered = _.contains(deliveredArray, userId);
              if (!delivered) {
                messageObj.delivered.push(userId);
              }
              message.save((err) => {
                if (err) {
                  reject(new Error('Failed to set  message as delivered  please try again later'));
                  return callback(new Error('Failed to set  message as delivered  please try again later'));
                }
